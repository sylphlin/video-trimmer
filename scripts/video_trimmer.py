...
def refine_speech_bounds_locked(audio, sr, t_first, t_last, total_dur, transcript="", pacing="auto"):
    ...
    speech_thresh = max(noise_floor * 3.5, 0.003)
    silence_thresh = max(noise_floor * 1.8, 0.0012)

    # 4. 句尾聲學邊界前向追蹤 (Acoustic Out-point Forward Tracking Only)
    # 核心防護準則：Whisper 字級標記點 t_last 已經是該字結束點，嚴禁向後倒退切入字詞內部！
    # 一律以 t_last 為絕對下限向後前向追蹤聲帶殘響自然衰減至底噪，徹底杜絕句末字 (如清塞音、輕聲詞) 被截斷。
    hangover_steps = 7  # 70ms 靜音平息期 (容忍塞音閉氣瞬斷)
    silent_count = 0
