...
- Adaptive noise floor tracking & 15ms equal-power audio micro-crossfades
...
def merge_whisper_segments_to_sentences(segments, max_gap=0.55, max_sentence_dur=20.0):
    ...
    for s in segments:
        ...
        is_s_japanese = bool(re.search(r'[\u3040-\u309f\u30a0-\u30ff]', s_text))
        is_curr_japanese = bool(re.search(r'[\u3040-\u309f\u30a0-\u30ff]', curr_text))
...
def transcribe_video_whisper(video_path, out_json_path, model_name="small"):
    ...
    if out_json.exists():
        ...
        if HAS_MLX_WHISPER:
            print(f"    本地調用 mlx-whisper ({model_name}) [Apple Silicon Metal GPU 原生加速] 進行微觀字級時間戳轉錄...")
        ...
        print("    [提示] 未安裝 mlx-whisper 或 faster-whisper，跳過本地字級轉錄。")
        return [], []
...
def refine_speech_bounds_locked(audio, sr, t_first, t_last, total_dur, transcript="", pacing="auto"):
    ...
    2. 開頭拍手打板防護 (Physical Clap Guard)。
    ...
    # 3. 局部環境底噪測繪 (Local Noise Floor Estimation)
    ...
    noise_floor = np.percentile(all_rms, 10) if all_rms else 0.0015
    speech_thresh = max(noise_floor * 3.5, 0.012)
    silence_thresh = max(noise_floor * 1.8, 0.004)

    # 4. 句尾聲學邊界雙向追蹤 (Acoustic Out-point Tracking)
    anchor_idx = min(len(audio) - win - 1, max(0, int(t_last * sr)))
    anchor_rms = np.sqrt(np.mean(audio[anchor_idx : anchor_idx + win] ** 2))

    hangover_steps = 7  # 70ms 靜音平息期
    true_speech_end = t_last

    if anchor_rms < silence_thresh:
        # 講者在 Whisper 標記點前已提前閉口 -> 向後回縮死寂
        b_steps = np.arange(t_last, max(t_first, t_last - 0.45), -0.01)
        for t in b_steps:
            idx = int(t * sr)
            ...
            if r >= speech_thresh:
                true_speech_end = t + 0.03
                break
    else:
        # 講者在 Whisper 標記點處仍在發音 (外語尾音/鼻音) -> 向前追蹤至真實落地點
        f_steps = np.arange(t_last, min(total_dur - 0.05, t_last + 1.2), 0.01)
        silent_count = 0
        for t in f_steps:
            idx = int(t * sr)
            if idx + win >= len(audio):
            ...
            # 真正防干擾：先出現靜音間隔，隨後又突然爆發外人說話/拍手脈衝
            if silent_count >= 5 and (r > speech_thresh * 2 or peak > 0.30):
                true_speech_end = t - (silent_count * 0.01) + 0.02
                break

            if r < silence_thresh:
                silent_count += 1
                if silent_count >= hangover_steps:
                    true_speech_end = t - (hangover_steps * 0.01) + 0.03
                    break
            else:
                silent_count = 0
                true_speech_end = t

    # 5. 依據 CPS 計算自然呼吸氣息與定格留白
    ...
    # 6. 句首起音聲學追蹤與拍手防護 (Acoustic In-point Snapping & Dead Air Trimming)
    ...
    if in_rms < speech_thresh:
        ...
        for t in np.arange(t_first, max(0.0, t_first - 0.15), -0.01):
            idx = int(t * sr)
            if idx < 0:
                break
            r = np.sqrt(np.mean(audio[idx : idx + win] ** 2))
    ...
    if last_clap is not None and last_clap < true_speech_start:
        final_in = max(last_clap + 0.10, target_in)
    else:
        final_in = target_in
    final_in = round(final_in, 2)
...
def render_cut_video(edl, video_path, out_mp4_path, crf=18):
    ...
    for i, c in enumerate(edl):
        dur = c["duration"]
        fade_d = min(0.015, max(0.005, dur / 4))
